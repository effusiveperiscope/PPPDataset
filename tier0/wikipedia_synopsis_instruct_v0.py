...
PRE_RESPONSE_TEMPLATES = [
    ...
    " of {mlp}, which is the {ep}th episode of the {s}th season: ",
    "I can provide a summary of the episode {episode} from {mlp},"
...
def gen_summary_question(season_num, episode_title, episode_num):
    ...
    return temp.format(episode = episode_title,
        ...
        ep_th = instruct_util.choose_nth(int(episode_num)), ep = episode_num,
        s_th = instruct_util.choose_nth(int(season_num)), s = season_num)
...
for s in range(1,3):
    ...
    for ep,x in enumerate(wiki_synopsis_list[idx]):
        ...
        instruct_dataset.append({"instruction": instruction,
            "input": "", "output": random.choice(PRE_RESPONSE_TEMPLATES)
            +x["synopsis"]})
